...
using AwfulNET.Phone.Resources;
...
namespace AwfulNET.Phone
{
    public partial class App : Application
    {
        ...
        /// <summary>
        /// Provides easy access to the current user's access token.
        /// </summary>
        /// <returns>The active account of the current user.</returns>
        public static UserAccount CurrentAccount { get; set; }

        /// <summary>
        /// Singleton for the app's main data model.
        /// </summary>
        ...
        public App()
        {
            ...
            // Intialize main data model.
            Main = new MainDataModel();

            // Initialize database
            UserContext = new UserDataContext(userDBConnectionString);
            UserContext.CreateIfNotExists();

            // Register for Device Network status changes
        ...
        // Check if user has a item pinned to her favorites.
        private void OnPinnedForumQuery(INotification<PinnedForumQuery> obj)
        {
            string id = obj.Value.UniqueID;
            obj.Value.IsPinned = CurrentAccount.HasUserFavoriteWithId(id);
        }
        ...
        private void OnDialogMessage(INotification<DialogMessage> obj)
        {
            MessageBox.Show(obj.Value.Message, obj.Value.Caption, MessageBoxButton.OK);
        }
        ...
        private void OnConfirmDialogMessage(INotification<ConfirmDialogMessage> obj)
        {
            var response = MessageBox.Show(obj.Value.Message, obj.Value.Caption, MessageBoxButton.OKCancel);
            if (response == MessageBoxResult.OK)
                obj.Value.Confirm(true);
            else
                obj.Value.Confirm(false);
        }

        // Show a message dialog box for now; maybe implement toast notifications later.
        private void OnToastMessage(INotification<ToastMessage> obj)
        {
            MessageBox.Show(obj.Value.Message,
                obj.Value.Caption == null ? string.Empty : obj.Value.Caption,
                MessageBoxButton.OK);
        }
        ...
        //
        // For more info on localizing Windows Phone apps see html://go.microsoft.com/fwlink/?LinkId=262072.
        //
